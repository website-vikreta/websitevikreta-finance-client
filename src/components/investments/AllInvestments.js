...
//import Item from './Items';
 const Investment = React.lazy(() => import('./Investments'));

const DateComponent = styled(DatePicker)`
    width: 250px
`;
export default function AllInvestments({ investments, setInvestments, render, setRender }) {
   ...
   const [filtr, setFilter] = useState({ filter: '_id', allActive: true, mutualActive: false, stockActive: false, fixedActive: false});
   const { filter, allActive, mutualActive, stockActive, fixedActive } = filtr;
   ...
   return (
      <div>
         ...
         <AppBar position="static" sx={{ backgroundColor: '#FFFFFF', boxShadow: 'none' }}>
            <Toolbar className='filterToolbar'>
               <div className="filterButtonWrapper">
                  ...
                  <button
                     className={`btn ${mutualActive ? 'btn-primary' : 'btn-secondary'}`}
                     onClick={(() => setFilter({ filter: 'Mutual Funds', mutualActive: true }))}>
                     Mutual Funds
                  </button>
                  <button
                     className={`btn ${stockActive ? 'btn-primary' : 'btn-secondary'}`}
                     onClick={(() => setFilter({ filter: 'Stock', stockActive: true }))}>
                     Stock
                  </button>
                  <button
                     className={`btn ${fixedActive ? 'btn-primary' : 'btn-secondary'}`}
                     onClick={(() => setFilter({ filter: 'Fixed Deposit', stockActive: true }))}>
                     Fixed Deposit
                  </button>
               </div>
         ...
         {/* Item Data Table  */}
         <Investment investments={investments} setInvestments={setInvestments} render={render} setRender={setRender} type={filter} dateFilter={val} startDate={startDate} endDate={endDate} />
      </div>
   );
}